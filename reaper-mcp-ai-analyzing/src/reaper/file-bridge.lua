...
local plugin_db = {
    -- Waves plugins (VST3 format in REAPER)
    waves = {
        ["cla-2a"] = "VST3: CLA-2A",
        ["cla2a"] = "VST3: CLA-2A",
        ["cla-3a"] = "VST3: CLA-3A",
        ...
        ["sslg"] = "VST3: SSL G-Channel",
        ["ssl-e"] = "VST3: SSL E-Channel",
        ["ssle"] = "VST3: SSL E-Channel",
        ["reneissance"] = "VST3: Renaissance Vox",
    ...
    fabfilter = {
        ...
        ["promb"] = "VST: Pro-MB",
        ["saturn"] = "VST: Saturn 2",
        ["saturn-2"] = "VST: Saturn 2",
        ["timeless"] = "VST: Timeless 3",
        ["timeless-3"] = "VST: Timeless 3",
        ["volcano"] = "VST: Volcano 3",
        ["volcano-3"] = "VST: Volcano 3",
    },
    ...
    reaper = {
        ...
        ["reaverb"] = "ReaVerb",
        ["reavocode"] = "ReaVocode",
        ["reasamplomatic"] = "ReaSamplOmatic5000",
    }
}
...
-- ===== NEW: Smart FX Add with Mono/Stereo detection =====
function handlers.add_fx_to_track_smart(params)
    ...
    if not track then
        ...
        return { 
            success = false, 
            error = string.format("Track index %d not found. Project has %d track(s) (valid indices: 0 to %d, track numbers: 1 to %d)", 
                track_index, track_count, math.max(0, track_count - 1), track_count)
        }
    end
    ...
    -- Resolve plugin name from database
    ...
    -- Helper to add mono/stereo variants (prioritize suffixed versions)
    local function add_variants(base_name)
        if is_mono then
            -- Mono track: try MONO suffix FIRST, then as-is
            if base_name:find("VST3:") then
                -- Waves format: VST3: CLA-2A -> VST3: CLA-2A MONO
                table.insert(fx_names_to_try, base_name .. " MONO")
            elseif base_name:find("VST:") then
                -- FabFilter format: VST: Pro-Q 3 -> VST: Pro-Q 3 Mono
                table.insert(fx_names_to_try, base_name .. " Mono")
            else
                -- Generic
                table.insert(fx_names_to_try, base_name .. " Mono")
            ...
            -- Stereo track: try STEREO suffix FIRST, then as-is
            if base_name:find("VST3:") and not base_name:find("STEREO") then
                -- Waves format: try VST3: CLA-2A STEREO first
    ...
    -- Also try the original input as fallback
