...
export class ReaperPluginParser {
   ...
   */
  async getAvailablePlugins(searchQuery?: string, maxResults: number = 50, forceRefresh: boolean = false): Promise<{
    plugins: string[];
    count: number;
    totalCount: number;
    ...
    // Use cache if valid (or force refresh)
    if (forceRefresh || !this.pluginCache || Date.now() - this.cacheTimestamp > this.CACHE_TTL) {
  ...
  private async parseAllPlugins(): Promise<string[]> {
    ...
    try {
      ...
      Logger.info(`[PluginParser] Parsed ${vstPlugins.length} VST plugins from ${this.vstIniPath}`);
    } catch (error) {
      Logger.error(`[PluginParser] Failed to parse VST plugins from ${this.vstIniPath}:`, error);
    }
    ...
    try {
      ...
      Logger.info(`[PluginParser] Parsed ${auPlugins.length} AU plugins from ${this.auIniPath}`);
    } catch (error) {
      Logger.error(`[PluginParser] Failed to parse AU plugins from ${this.auIniPath}:`, error);
    }

    // Sort alphabetically
    const sorted = Array.from(plugins).sort();
    Logger.info(`[PluginParser] Total unique plugins: ${sorted.length}`);
    return sorted;
  }
   ...
   * - WaveShell1_VST3_16.0.vst3<1072405404=00D58C9D6616DC01,1072405404{5653544C41324D636C612D3261206D6F,CLA-2A Mono (Waves)
   */
  private async parseVstIni(): Promise<string[]> {
    try {
      ...
      for (const line of lines) {
        ...
        // Parse format: filename.vst/vst3 or filename.vst3<id=...
        // Find the first '=' after the ID section
        const equalIndex = trimmed.indexOf('=');
        if (equalIndex === -1) continue;

        // Get the part before '=' to extract filename
        // Format can be: filename.vst3<id=... or filename.vst=...
        let beforeEqual = trimmed.substring(0, equalIndex);
        const afterEqual = trimmed.substring(equalIndex + 1);
        
        // Handle VST3 format: WaveShell1_VST3_16.0.vst3<1072405404=...
        // Extract actual filename (before the '<' if present)
        let filename: string;
        const angleIndex = beforeEqual.indexOf('<');
        if (angleIndex !== -1) {
          filename = beforeEqual.substring(0, angleIndex).toLowerCase();
        } else {
          filename = beforeEqual.toLowerCase();
        }
      ...
      Logger.info(`[PluginParser] VST plugins: ${vstCount}, VST3 plugins: ${vst3Count}`);
      return plugins;
    } catch (error) {
      if ((error as any).code === 'ENOENT') {
        Logger.error(`[PluginParser] VST plugin cache not found: ${this.vstIniPath}`);
      }
      throw error;
    }
  }
  ...
  private async parseAuIni(): Promise<string[]> {
    try {
      ...
      if ((error as any).code === 'ENOENT') {
        Logger.error(`[PluginParser] AU plugin cache not found: ${this.auIniPath}`);
      }
      throw error;
    }
  }
